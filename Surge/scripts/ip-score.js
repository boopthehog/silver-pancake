...
// Read API keys from the module argument
const IPQS_API_KEY = arg.ipqs_key || ''
// const SCAMALYTICS_API_KEY = arg.scamalytics_key || ''; // Uncomment when you have a key
...
!(async () => {
  ...
  // First, get the basic IP info. The IP address from here will be used for other API calls.
  let baseInfo = await getIpScoreInfo()
  const ip = $.lodash_get(baseInfo, 'ip')
  if (!ip) {
    throw new Error('Could not retrieve IP address from ip-score.com')
  }
  title = `${ip}`

  // Now, fetch from all sources concurrently
  let [ipqsInfo /*, scamalyticsInfo */] = await Promise.all([
    getIPQSInfo(ip, IPQS_API_KEY),
    // getScamalyticsInfo(ip, SCAMALYTICS_API_KEY) // Uncomment when you have a key
  ])

  // --- Format IPQS Info ---
  let ipqsContent = ''
  if (ipqsInfo && ipqsInfo.success) {
    const score = ipqsInfo.fraud_score
    const isProxy = ipqsInfo.proxy ? '✅' : '🚫'
    const isVpn = ipqsInfo.vpn ? '✅' : '🚫'
    const isTor = ipqsInfo.tor ? '✅' : '🚫'
    const isBot = ipqsInfo.bot_status ? '✅' : '🚫'
    ipqsContent = `IPQS Score: ${score} (Proxy: ${isProxy}, VPN: ${isVpn}, Tor: ${isTor}, Bot: ${isBot})\n`
  } else if (IPQS_API_KEY) {
    ipqsContent = `IPQS: ${ipqsInfo.message || 'Error'}\n`
  }

  // --- Format Scamalytics Info (Placeholder) ---
  // let scamalyticsContent = '';
  // if (scamalyticsInfo && scamalyticsInfo.status === 'ok') {
  //   scamalyticsContent = `Scamalytics Score: ${scamalyticsInfo.score}\n`;
  // } else if (SCAMALYTICS_API_KEY) {
  //   scamalyticsContent = `Scamalytics: Error\n`;
  // }

  // --- Format ip-score.com Info ---
  const blacklistsObj = $.lodash_get(baseInfo, 'blacklists') || {}
  let blacklists = []
  Object.keys(blacklistsObj).forEach(key => {
    blacklists.push(`${key.toUpperCase()}: ${blacklistsObj[key]}`)
  })
  blacklists = blacklists.length > 0 ? `Blacklists:\n${blacklists.join('\n')}\n` : ''

  let isp = []
  ;['isp', 'org', 'asn'].forEach(key => {
    isp.push(`${key.toUpperCase()}: ${$.lodash_get(baseInfo, key) || ' - '}`)
  })
  isp = isp.length > 0 ? `${isp.join('\n')}\n` : ''

  let geo1 = []
  ;['country', 'region', 'city'].forEach(key => {
    if ($.lodash_get(baseInfo, `geoip1.${key}`)) {
      geo1.push(`${key.toUpperCase()}¹: ${$.lodash_get(baseInfo, `geoip1.${key}`) || ' - '}`)
    }
  })
  ...
  let geo2 = []
  ;['country', 'region', 'city'].forEach(key => {
    if ($.lodash_get(baseInfo, `geoip2.${key}`)) {
      geo2.push(`${key.toUpperCase()}²: ${$.lodash_get(baseInfo, `geoip2.${key}`) || ' - '}`)
    }
  })
  geo2 = geo2.length > 0 ? `${geo2.join('\n')}\n` : ''

  // Combine all content, putting fraud scores at the top
  content = `${ipqsContent}${geo1}${geo2}${blacklists}${isp}执行时间: ${new Date().toTimeString().split(' ')[0]}`
...
async function getIPQSInfo(ip, apiKey) {
  if (!apiKey) return { success: false, message: 'API key not provided.' }
  if (!ip) return { success: false, message: 'IP address not provided.' }
...
async function getScamalyticsInfo(ip, apiKey) {
  if (!apiKey) return { status: 'error', message: 'API key not provided.' }
  if (!ip) return { status: 'error', message: 'IP address not provided.' }

  // Scamalytics API endpoint and structure is an assumption.
  // You will need to verify their actual API documentation.
  try {
    const res = await $.http.get({
      url: `https://api.scamalytics.com/ip/${apiKey}?ip=${ip}`,
    })
    return JSON.parse(res.body)
  } catch (e) {
    $.logErr(e)
