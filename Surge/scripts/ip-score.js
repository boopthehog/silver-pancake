...
if (typeof $argument != 'undefined') {
  // FIX: Use a comma "," to split arguments. This is the correct, safe method for Surge.
  arg = Object.fromEntries($argument.split(',').map(item => item.split('=')))
}

// Read API keys from the module argument
const IPQS_API_KEY = arg.ipqs_key || ''
// const SCAMALYTICS_API_KEY = arg.scamalytics_key || ''; // Placeholder for future use
...
!(async () => {
  ...
  let [ipqsInfo] = await Promise.all([
    getIPQSInfo(ip, IPQS_API_KEY),
    // getScamalyticsInfo(ip, SCAMALYTICS_API_KEY) 
  ])
  ...
  if (ipqsInfo && ipqsInfo.success) {
    ...
    ipqsContent = `IPQS Score: ${score} (Proxy:${isProxy} VPN:${isVpn} Tor:${isTor})\n`
  } else if (IPQS_API_KEY) {
    ipqsContent = `IPQS: ${ipqsInfo.message || 'Error checking status'}\n`
  }
...
async function getIPQSInfo(ip, apiKey) {
  if (!apiKey || apiKey === 'YOUR_KEY_HERE') return { success: false, message: 'API key not set.' }
...
//... (The rest of the script is identical to the previous version and can be included here)
// prettier-ignore
